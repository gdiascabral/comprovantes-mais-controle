...
def _versao_na_tela():
    ...
    alvo = next((n for n in arvore.body
                 if isinstance(n, ast.FunctionDef) and n.name == "_versao_na_tela"),
                None)
    assert alvo is not None, "`_versao_na_tela` sumiu do comprovantes_app.py"
    ns: dict = {}
    exec(compile(ast.Module(body=[alvo], type_ignores=[]),
                 "comprovantes_app.py", "exec"), ns)
    return ns["_versao_na_tela"]


@pytest.mark.parametrize("completa, na_tela", [
    ("v2.0.201", "v2.0.201"),
    ("v2.0.9", "v2.0.9"),
    ("2.0.108", "v2.0.108"),    # sem o "v" na frente
    (" v2.0.201\n", "v2.0.201"),
    ("v2.0", "v2.0"),
])
def test_a_tela_mostra_o_numero_inteiro_da_versao(completa, na_tela):
    """Pedido do dono em 14/09/2026: ver o número inteiro. Até então a tela
    mostrava só "v2.0" e o número de build ficava escondido na dica — e é
    justamente ele que diz se a máquina já pegou a versão liberada."""
    assert _versao_na_tela()(completa) == na_tela
...
def test_sem_versao_a_tela_nao_escreve_nada(entrada):
    ...
    assert _versao_na_tela()(entrada) == ""


@pytest.mark.parametrize("entrada", ["vabc", "qualquer coisa"])
def test_versao_estranha_sai_inteira(entrada):
    """Lixo entra e sai inteiro: é a pista de que o `versao.txt` veio errado."""
    assert _versao_na_tela()(entrada) == entrada.strip()
