...
def monta_descricao(item: dict, files, comentario: str = "", overview=None) -> str:
    cc = " | ".join((c.get("workName") or "").strip()
                    for c in (item.get("costCentreDetails") or []) if c).strip(" |")
...
def conferir_documento(item: dict, files, textos) -> tuple[str, bool]:
...
def montar_registros(lancamentos, anexos: dict, overviews: dict, textos: dict,
                     incluir=(), excluir=(), pix_reembolso=None) -> dict:
    """{conta: [linha, ...]}.
    ...
    for item in lancamentos:
        ...
        conferencia, divergiu = (conferir_documento(item, files, do_item)
        ...
        if cls == "PAGAR_PARA":
            tipo = "Pix"
            dados = pix_do_reembolso(files, item, pix_reembolso)
            obs = ("Reembolso — pagar a chave do aviso, NÃO o pix do cadastro"
                   if dados else
                   f"Reembolso para '{nome_do_reembolso(files) or '?'}' — chave não "
            ...
            dados = extrair_linha_digitavel(textos.get((pdf or {}).get("downloadUrl") or ""))
            if not dados:
                obs = ("Boleto em imagem — preencher manual" if pdf else
                       "Nenhum anexo de boleto — conferir no ERP")
...
def gerar_excel(registros: dict, caminho: Path, log=print) -> Path:
    wb = Workbook()
    wb.remove(wb.active)
