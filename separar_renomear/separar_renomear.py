...
# rótulo e o valor na mesma linha SEM dois-pontos: "Pago R$ 1.234,56",
# "Valor R$ 1.000,00", "Pagamento 08/09/2026", "Observação OBRA QD 12 ...".
...
def _descricao(t, banco):
    ...
    for i, l in enumerate(L):
        # Com ou sem ":". O comprovante comum do Sicoob escreve "Observação OBRA
        # EXEMPLO QD 12 LT 07 NF 1234 OC 5678", sem dois-pontos, e a exigência do
        ...
        if m:
            ...
            if not dois_pontos:
                ...
                # abaixo ("OBRA EXEMPLO QD 9 LT 99 DEVOLUÇÃO…" / "Observação" /
                # "CS 02"), e é assim que o pdfplumber e o OCR a devolvem.
                if banco == 'SICOOB':
...
def _conta_do_bloco(t, bloco):
    ...
    do Sicoob: `Conta 00.000-0 / EMPRESA EXEMPLO LTDA`, até 3 linhas abaixo
...
def _campos_rotulado(t):
    ...
    if banco == 'INTER':
        ...
        if tipo == 'CONVENIO' and not dest:
            ...
            # convênio ("Convênio CONCESSIONARIA GO"), que é o que identifica a conta
...
# ("12BLT 07"), lê o O do OC como zero e o C como € ("0€ 1234", "0C 5678") e
# cola a sigla da obra no número ("AB12"). Sem o "OC 1234" o matcher não acha
# a OC (`OC\s*(\d+)`), que é o sinal mais forte do casamento.
RE_OC_TORTO = re.compile(r"(?<![A-Za-z0-9])[0O]€?[C€](?=\s*\d{3,})")
# só depois de número, ou do número com a letra do lote ("12BLT 07"): "VOLT
...
# e o psm 6 lê o Q do QD como G ou O ("GD 12B", "OD 12B")
...
def _serve_de_nome(cand) -> bool:
    ...
    Três letras bastam: há recebedor com nome de três letras ("Ivo"), e a regra
    antiga de mais de 4 caracteres pulava o nome e pegava a linha de cima —
    no Pix do Inter lido por OCR, o horário ("10h00") ou o identificador da
    ...
    # no Pix do Inter é a data ("Quinta, 01/01/2026")
