...
- `pagamentos_dia/pagamentos_frame.py` — aba Pagamentos do Dia, em 3 passos
  ...
  **O passo 2 tem DUAS fases desde 14/09/2026** (pedido do dono: "trazer as
  ...
  cadastro de contas a janela abre igual, com um aviso no topo. Quatro
  ...
  caminho e não consulta histórico) e a linha nasce DESMARCADA, como na
  conferência da remessa; se o registro cai no meio da análise, o que o lote
  já respondeu fica, e a linha não conferida diz "não conferi se já saiu em
  remessa" em âmbar — nunca verde por falta de pergunta; o reembolso mostra
  em QUEM RECEBE a pessoa do aviso "(reembolso de <favorecido>)"; e Parar
  durante a leitura não abre a janela (leitura pela metade transforma o
  boleto dentro da NF em Pix do cadastro), enquanto anexo que não baixou vira
  aviso no topo. O período da planilha é o do "1. Buscar", não o da tela no
  clique. **O "já saiu?" do registro usa `remessa!inner(...)`** (14/09/2026,
  `nuvem/registro.py`): sem o `!inner` o item de maior id era escolhido antes
  de o filtro de estado valer, e um reenvio numa remessa DESCARTADA escondia
  o envio antigo numa remessa VIVA — o boleto voltava marcável no "Gerar
  remessa". A sintaxe foi conferida contra o PostgREST real (passa do parse
  e esbarra na permissão de anon, como a consulta antiga).
  ...
  **As duas listas de conferência são UMA tabela cada (11/09/2026).** A
  ...
  (`tests/test_listas_de_conferencia.py` e `tests/test_confirmacao.py`): a da
  confirmação mora em `pagamentos_dia/confirmacao.py`, pura
  (`grupos_da_confirmacao`, `situacao_da_linha`, `estado_na_tela`, `resumo`,
  `nao_confirmados`); `nsa_previstos`, `resumo_da_conferencia` e `aplicar_marcas` no
  ...
  **"Gerar HTML dos pagamentos" é PROVISÓRIO** (11/09/2026, até a remessa
  ...
  em fronteira de palavra e nunca a NF, a OC ou o C/M. **Para remover**: apagar os dois módulos e o teste, o método
